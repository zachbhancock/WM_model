...
#get zoomed bathymap
maxlat <- max(gbif$y, genetic$y) + 10
minlat <- min(gbif$y, genetic$y) - 30
maxlon <- max(gbif$x, genetic$x) + 10
minlon <- min(gbif$x, genetic$x) - 10
...
main.plot <- ggplot() + 
  ...
  #add genetic points
  geom_point(data = genetic, aes(x=x, y=y, color = "magenta"), alpha = 1, size = 1) +
